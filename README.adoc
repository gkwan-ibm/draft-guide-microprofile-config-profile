...
The application that you will be working with is a `query` service. It fetches properties from the `system` microservice. You'll learn different ways to retreive configuration for accessing the `system` service using MicroProfile Config APIs.
...
A [hotspot=basicRegistry file=0]`basicRegistry` element is configured in the `system` microservice. Only the registered user is authorized to access the `system` microservice. Note that this `basicRegistry` element is a very simple case for learning purposes. For more information about the different user registries, see the https://openliberty.io/docs/latest/user-registries-application-security.html[User registries documentation^].
