...
:page-seo-description: A tutorial and example on how to access configuration details and to aggregate configuration properties into a CDI bean for Java microservices using MicroProfile Config APIs.
...
Learn how to use MicroProfile Config APIs to access configuration details and to aggregate configuration properties into a CDI bean.
...
The application that you will be working with is a `query` service. It fetches properties from the `system` microservices. The `system` microservice will use different port and require different user when running in the testing and development environments. You'll learn different ways to retreive configuration for  the `system` service using MicroProfile Config APIs.
...
After you are finished checking out the application, stop both the `query` service by pressing `CTRL+C` in the command-line sessions where you ran it. Alternatively, you can run the following goals from the `finish/query` directory in another command-line session:
