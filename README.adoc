...
add the [hotspot=userProperties file=0]`system.userPassword` property that is constructed by `system.user` and `system.password`
...
SystemResource.java
[source, java, linenums, role="code_column hide_tags=properties,contactEmail"]
----
...
Use the [hotspot=authHeader file=1]`systemUserPassword` to construct the authorization header.
