...
Point your browser to the http://localhost:9085/query/systems/localhost URL to test out the application again. The `java.version` and `os.name` from the `system` service are displayed.
...
Check out the service that you created at the http://localhost:9085/query/systems/localhost URL. The `java.version` and `os.name` from the `system` service are displayed.
