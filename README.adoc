...
Now, navigate to the `start/query` directory to create configuration profile in different ways.
...
Define the [hotspot=system file=0]`system.*` properties in the [hotspot file=0]`microprofile-config-development.properties` file according to the values from the [hotspot=developmentProperties file=1]`development` profile of the `system` service.
...
Remove all the [hotspot=development file=2]`%development.*` properties from the [hotspot file=2]`microprofile-config.properties` file.
