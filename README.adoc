...
In the `testing` environment, the configuration from the [hotspot=testingProperties file=1]`pom.xml` file is used. You can find out that the [hotspot=httpEndpoint hotspot=webApplication file=0]`system` service is running on port [hotspot=port file=1]`9080` with context root [hotspot=context.root file=1]`system/test`. In addition, a basic [hotspot=user file=0]`user` registry is configured with the username [hotspot=username file=1]`bob` and the password [hotspot=password file=1]`bobpwd` as the authorized access to the resources from other services.
