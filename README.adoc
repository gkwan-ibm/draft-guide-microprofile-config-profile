...
The application that you will be working with is a `query` service. It communicates with the `system` service to fetch the system property information of your local JVM. You'll add configuration properties to the `query` service by using Config Profiles according to the configurations required to access different environments of the `system` service.
