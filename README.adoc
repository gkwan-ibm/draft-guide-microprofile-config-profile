...
* http://localhost:9085/query/systems/localhost URL. You are expected to see the current OS and Java properties in JSON format. The URL retrieves the system property information for the `localhost` hostname by making a request to the `system` service at `\http://localhost:<system.httpPort>/<system.contextRoot>/property/{property}`. 

* http://localhost:9085/query/config/contact URL. You are expected to see the source name, source ordinal, and value of the `query.contactEmail` property.

* http://localhost:9085/query/config URL. You are expected to see all registered configuration sources and properties of the running `query` service.

* http://localhost:9085/query/config/system URL. You are expected to see all the configuration properties prefixed with `system.`
...
A [hotspot=basicRegistry file=1]`basicRegistry` element is configured in the `system` microservice. Only the registered user is authorized to access to the `system` microservice. Note that this `basicRegistry` element is a very simple case for learning purposes. For more information about the different user registries, see the https://openliberty.io/docs/latest/user-registries-application-security.html[User registries documentation^].

Add the [hotspot=userPassword file=0]`system.userPassword` configuration property to the `microprofile-config.properties` file. The [hotspot=userPassword file=0]`system.userPassword` property is expanded to `alice:alicepwd`.
...
Replace the injection of the `system.user` and `system.password` properties with the [hotspot=userPassword file=2]`system.userPassword` property, and use the [hotspot=authHeader file=2]`systemUserPassword` variable to construct the authorization header.
