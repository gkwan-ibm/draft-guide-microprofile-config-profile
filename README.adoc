...
Before testing the application in the `testing` environment, you need to update the `microprofile-config.properties` file with the new `system.userPassword`, `system.properties`, `role`, and `query.*` properties that were added in the previous section.
