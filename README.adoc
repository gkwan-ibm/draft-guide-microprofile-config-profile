...
The `/query/systems/{hostname}` endpoint constructs the [hotspot=authHeader file=1]`authHeader` authentication header with the [hotspot=user file=1]`system.user` and [hotspot=password file=1]`system.password` properties to access the `system` microservice. 
